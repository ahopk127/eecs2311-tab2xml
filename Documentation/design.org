...
** Back End Maintenance
  ...
  Sample Maintenance for Adding Bend notation:
   ...
   3. Define the rule at after the location the ~hampullchain~ rule is defined.
   4. ie. ~bend~ : ~fret~'b'~fret?~ is a sample rule.
   5. Add any data model classes as needed.
   6. Add a corresponding ~visit()~ method in ~ExtractStringItems~ to collect the bend notes.
