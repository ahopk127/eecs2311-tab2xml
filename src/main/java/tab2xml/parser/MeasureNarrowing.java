...
public final class MeasureNarrowing {
	...
	static final class StringPosition implements Comparable<StringPosition> {
		...
		public static final StringPosition fromIndex(String string, int index) {
			if (!string.endsWith(ROW_END))
				return fromIndex(string + ROW_END, index);
	 ...
	 */
	private static final Pattern MEASURE_LINE = Pattern
			.compile("^.{0,2}\\|.*-.*\\|$");
	...
	private static String[] getImportantLines(String textTab) {
		...
		// add a terminating newline if it isn't there
		// (since this is a list of lines, a newline is just an empty string)
		if (!filteredLines.isEmpty()
				&& !filteredLines.get(filteredLines.size() - 1).equals("")) {
			filteredLines.add("");
		}
	...
	/**
	 * "Linearizes" a text tab, moving all of its "lines" into one. If the
	 * provided text tab does not end in a newline, the returned one will.
	 *
