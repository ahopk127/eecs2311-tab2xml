...
public final class XMLMetadata {
	 ...
	 *         explicitly mapped to time signatures will not be present in the
	 *         returned map.
	 * @since 2021-03-31
	 */
	public final Map<IntRange, TimeSignature> getTimeSignatureRanges() {
		if (this.timeSignatureRanges == null) {
			...
			final int maxValue = this.timeSignatures.keySet().stream().mapToInt(Integer::intValue).max().orElse(0);
