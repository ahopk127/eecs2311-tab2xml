...
public final class XMLMetadata {
	...
	public static final class Builder {
		...
		public XMLMetadata build() {
			return new XMLMetadata(this.title, this.composer,
					...
					this.maxMeasure <= 0
							? this.timeSignatures.keySet().stream()
									.mapToInt(Integer::valueOf).max().orElse(1)
							: this.maxMeasure);
		}
	...
	public static final XMLMetadata fromDefaultTitle() {
		return fromTitle(DEFAULT_TITLE);
	}
	...
	private XMLMetadata(String title, Optional<String> composer,
			SortedMap<Integer, TimeSignature> timeSignatures, int maxMeasure) {
		this.title = title;
	 ...
	 *         not explicitly mapped to time signatures by the builder will be
	 *         present and mapped to null in the returned map.
	 * @since 2021-03-31
	 */
