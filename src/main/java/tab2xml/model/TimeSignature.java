...
public final class TimeSignature {
	...
	private static final Pattern TIME_SIGNATURE = Pattern.compile("([+-]?\\d+)" + SIGNATURE_SEPARATOR + "([+-]?\\d+)");

	/**
	 * Gets a time signature from a string in the same format as produced by
	 * {@link #toString}
	 *
	 * @return time signature, or empty Optional if string is not in correct format
	...
	public boolean equals(Object obj) {
		...
		return this.lowerNumeral == other.lowerNumeral && this.upperNumeral == other.upperNumeral;
	}
	...
	/**
	 * Returns a string representing this time signature. It will be composed of the
	 * decimal string representation of the upper numeral, then the character ':',
	 * then the lower numeral, with no spaces separating them. For example, the
	 * string representation of {@link #COMMON_TIME} is "4:4".
	 */
	@Override
